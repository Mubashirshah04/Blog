...
export default function AdminStudio() {
      ...
      try {
        ...
        if (analytics && analytics.length > 0) {
          ...
          analytics.forEach(item => {
            ...
            if (diffDays < 7) daily[6 - diffDays]++;
          });
          setDailyStats(daily);
        }

        // --- Process Posts Stats (Even if 0 views) ---
  ...
  for(let i=6; i>=0; i--) {
     ...
     labelDays.push(days[d.getDay() === 0 ? 6 : d.getDay() - 1]);
  }
      ...
      <div className={styles.statsGrid}>
        <div className={styles.statCard}>
          <div className={styles.statHeader}>
            <span className={styles.statTitle}>Real-time Views</span>
            <span className={styles.statIcon}>📈</span>
          </div>
          <div className={styles.statValue}>{totalViews.toLocaleString()}</div>
          <div className={`${styles.statTrend} ${styles.trendUp}`}>↑ Trending Up</div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statHeader}>
